...
return new class extends Migration
{
    ...
    {
        Schema::create('accounting_purchases', function (Blueprint $table): void {
            $table->id();
            ...
            $table->index(['company_site_id', 'status'], 'acct_purchases_site_status_idx');
            $table->index(['company_site_id', 'purchase_date'], 'acct_purchases_site_date_idx');
            $table->index(['company_site_id', 'supplier_id'], 'acct_purchases_site_supplier_idx');
        });

        Schema::create('accounting_purchase_lines', function (Blueprint $table): void {
            $table->id();
            ...
            $table->index(['purchase_id', 'line_type'], 'acct_purchase_line_type_idx');
        });

        Schema::create('accounting_purchase_payments', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('purchase_id')->constrained('accounting_purchases')->cascadeOnDelete();
            ...
            $table->foreignIdFor(User::class, 'paid_by')->nullable()->constrained('users')->nullOnDelete();
            ...
            $table->index(['purchase_id', 'payment_date'], 'acct_purchase_payment_date_idx');
            $table->index(['payment_method_id', 'payment_date'], 'acct_purchase_payment_method_date_idx');
        });
    }
    ...
    public function down(): void
    {
        ...
        Schema::dropIfExists('accounting_purchase_lines');
