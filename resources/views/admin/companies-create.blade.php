...
<head>
    ...
    <title>{{ __('admin.new_company') }} | {{ config('app.name', 'EXAD ERP') }}</title>
...
<body>
    @php
        ...
        $oldPhones = old('phones', [['label' => '', 'phone_number' => '']]);
        $oldAccounts = old('accounts', [['bank_name' => '', 'account_number' => '', 'currency' => '']]);
    @endphp
    ...
    <div class="dashboard-shell main-shell" data-theme="light">
        ...
        <main class="dashboard-main">
            ...
            <section class="dashboard-content company-form-page">
                <form class="admin-form company-create-form" method="POST" action="{{ route('admin.companies.store') }}" enctype="multipart/form-data" novalidate>
                    @csrf
                    ...
                    <section class="form-section">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select id="companySubscription" name="subscription_id" class="form-select @error('subscription_id') is-invalid @enderror" data-company-subscription data-required-message="{{ __('admin.required_user_subscription') }}">
                                    ...
                                    @foreach ($subscriptions as $subscription)
                                        <option value="{{ $subscription->id }}" @selected((string) old('subscription_id') === (string) $subscription->id)>{{ $subscription->name }} - {{ __('admin.'.$subscription->type) }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 company-admin-field" data-company-admin-wrapper @if (! old('subscription_id')) hidden @endif>
                                <label for="companyAdmin" class="form-label">{{ __('admin.administrator') }} *</label>
                                <select id="companyAdmin" name="admin_id" class="form-select @error('admin_id') is-invalid @enderror" data-company-admin data-required-message="{{ __('admin.required_company_admin') }}" @disabled(! old('subscription_id'))>
                                    <option value="">{{ __('admin.choose_admin') }}</option>
                                    @foreach ($admins as $admin)
                                        <option value="{{ $admin->id }}" data-subscription-id="{{ $admin->subscription_id }}" @selected((string) old('admin_id') === (string) $admin->id)>{{ $admin->name }} - {{ $admin->email }}</option>
                                    @endforeach
                                </select>
                    ...
                    <section class="form-section">
                        ...
                        <div class="row g-3">
                            <div class="col-md-8">
                                ...
                                <input id="companyName" name="name" type="text" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" data-required-message="{{ __('admin.required_company_name') }}">
                            ...
                            <div class="col-12">
                                ...
                                <input id="companySlogan" name="slogan" type="text" class="form-control" value="{{ old('slogan') }}">
                            </div>
                            <div class="col-md-4"><label for="companyRccm" class="form-label">{{ __('admin.rccm') }}</label><input id="companyRccm" name="rccm" type="text" class="form-control" value="{{ old('rccm') }}"></div>
                            <div class="col-md-4"><label for="companyIdNat" class="form-label">{{ __('admin.id_nat') }}</label><input id="companyIdNat" name="id_nat" type="text" class="form-control" value="{{ old('id_nat') }}"></div>
                            <div class="col-md-4"><label for="companyNif" class="form-label">{{ __('admin.nif') }}</label><input id="companyNif" name="nif" type="text" class="form-control" value="{{ old('nif') }}"></div>
                        </div>
                    </section>
                    ...
                    <section class="form-section">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input id="companyEmail" name="email" type="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}" data-required-message="{{ __('admin.required_company_email') }}" data-email-message="{{ __('admin.invalid_user_email') }}">
                            ...
                            <div class="col-md-6"><label for="companyWebsite" class="form-label">{{ __('admin.website') }}</label><input id="companyWebsite" name="website" type="url" class="form-control @error('website') is-invalid @enderror" value="{{ old('website') }}" placeholder="https://..."></div>
                            <div class="col-12"><label for="companyAddress" class="form-label">{{ __('admin.address') }}</label><input id="companyAddress" name="address" type="text" class="form-control" value="{{ old('address') }}"></div>
                        </div>
                    </section>
                        ...
                        <div class="dynamic-list" data-account-list>
                            @foreach ($oldAccounts as $index => $account)
                                <div class="dynamic-row dynamic-row-accounts">
                                    ...
                                    <input name="accounts[{{ $index }}][currency]" type="text" class="form-control" value="{{ $account['currency'] ?? '' }}" placeholder="{{ __('admin.currency') }}">
                                    <button type="button" class="table-button table-button-delete" data-remove-row aria-label="{{ __('admin.delete') }}"><i class="bi bi-trash" aria-hidden="true"></i></button>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <section class="form-section visual-section">
                        ...
                        <div class="logo-upload-row">
                            <span class="logo-preview" data-logo-preview>?</span>
                            <div>
    ...
    <template id="accountRowTemplate">
        <div class="dynamic-row dynamic-row-accounts">
            ...
            <input data-name="accounts[__INDEX__][currency]" type="text" class="form-control" placeholder="{{ __('admin.currency') }}">
            <button type="button" class="table-button table-button-delete" data-remove-row aria-label="{{ __('admin.delete') }}"><i class="bi bi-trash" aria-hidden="true"></i></button>
        </div>
    </template>
