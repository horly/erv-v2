...
<body>
    @php
        ...
        $formAction = $isEditingCompany ? route('main.companies.update', $company) : route('main.companies.store');
    ...
    <div class="main-shell" data-theme="light">
                ...
                <div class="language-menu">
                    ...
                    <div class="language-dropdown" id="languageDropdown" aria-labelledby="languageButton">
                        <a class="language-option {{ $currentLocale === 'fr' ? 'active' : '' }}" href="{{ route('locale.switch', 'fr') }}">
                            ...
                            @if ($currentLocale === 'fr')<i class="bi bi-check-lg language-check" aria-hidden="true"></i>@endif
                        </a>
                        <a class="language-option {{ $currentLocale === 'en' ? 'active' : '' }}" href="{{ route('locale.switch', 'en') }}">
                            ...
                            @if ($currentLocale === 'en')<i class="bi bi-check-lg language-check" aria-hidden="true"></i>@endif
                        </a>
                    </div>
                </div>
        ...
        <main class="content-wrap company-form-page">
            <section class="page-heading">
                <div>
                    <h1>{{ $pageTitle }}</h1>
                    <p><a class="breadcrumb-link" href="{{ route('main') }}">{{ __('main.title') }}</a> / {{ $isEditingCompany ? __('admin.edit') : __('admin.create') }}</p>
                </div>
            </section>
            ...
            <form class="admin-form company-create-form" method="POST" action="{{ $formAction }}" enctype="multipart/form-data" novalidate>
                ...
                <section class="form-section">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="companyCountry" name="country" class="form-select @error('country') is-invalid @enderror" data-required-message="{{ __('admin.required_country') }}">
                                ...
                                @foreach ($countries as $country)
                                    ...
                                    <option value="{{ $country['name_fr'] }}" @selected(old('country', $company?->country ?? 'Congo (RDC)') === $country['name_fr'])>{{ $countryName }} ({{ $country['phone_code'] }} - {{ app()->getLocale() === 'en' ? 'VAT' : 'TVA' }} {{ number_format($country['vat_rate'], 2, ',', ' ') }}%)</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12"><label for="companySlogan" class="form-label">{{ __('admin.slogan') }}</label><input id="companySlogan" name="slogan" type="text" class="form-control" value="{{ old('slogan', $company?->slogan) }}"></div>
                                ...
                                <div>
                                    <select name="accounts[{{ $index }}][currency]" class="form-select @error('accounts.'.$index.'.currency') is-invalid @enderror" aria-label="{{ __('admin.currency') }}">
                                        ...
                                        @foreach ($currencies as $code => $currency)
                                            <option value="{{ $code }}" @selected(($account['currency'] ?? '') === $code)>{{ $currencyLabel($code, $currency) }}</option>
                                        @endforeach
                                    </select>
                ...
                <div class="page-form-actions">
                    <a class="modal-cancel" href="{{ route('main') }}">{{ __('admin.cancel') }}</a>
