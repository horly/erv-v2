                            ...
                            <tbody>
                                @forelse ($clients as $client)
                                    @php
                                        ...
                                        $primaryPhone = $client->isCompany() ? ($client->contacts->first()?->phone ?? '-') : ($client->phone ?: '-');
                                        $primaryEmail = $client->isCompany() ? ($client->contacts->first()?->email ?? '-') : ($client->email ?: '-');
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $primaryEmail }}</td>
                                        <td>
                                            @if ($clientPermissions['can_update'] || $clientPermissions['can_delete'])
                                                <div class="table-actions">
                                                    ...
                                                    @if ($clientPermissions['can_delete'])
                                                        <form method="POST" action="{{ route('main.accounting.clients.destroy', [$company, $site, $client]) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" class="table-button table-button-delete" aria-label="{{ __('admin.delete') }}" data-delete-trigger data-delete-title="{{ __('main.delete_client_title') }}" data-delete-text="{{ __('main.delete_client_text', ['name' => $client->name]) }}" data-delete-confirm="{{ __('admin.delete_user_confirm') }}" data-delete-cancel="{{ __('admin.delete_user_cancel') }}">
                                ...
                                @empty
                                    <tr class="empty-row"><td colspan="7">{{ __('main.no_clients') }}</td></tr>
                                @endforelse
