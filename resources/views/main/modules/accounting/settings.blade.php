...
<head>
    ...
    <title>{{ __('main.module_settings') }} | {{ app_brand_name() }}</title>
...
<body class="accounting-module-body">
    ...
    <div class="dashboard-shell main-shell accounting-shell" data-theme="light">
        @include('main.modules.partials.accounting-sidebar', ['activeAccountingPage' => 'settings'])

        <main class="dashboard-main">
            @include('main.modules.partials.accounting-topbar', ['title' => __('main.module_settings')])

            <section class="dashboard-content module-dashboard-page accounting-settings-page">
                ...
                <section class="page-heading">
                    <div>
                        <h1>{{ __('main.module_settings') }}</h1>
                        <p>{{ __('main.module_settings_subtitle') }}</p>
                    </div>
                </section>
                ...
                <form method="POST" action="{{ route('main.accounting.settings.update', [$company, $site]) }}" class="accounting-settings-form">
                    @csrf
                    @method('PUT')
                    ...
                    <section class="settings-panel">
                        <header class="settings-panel-heading">
                            ...
                            <div>
                                <h2>{{ __('main.pdf_print_identity') }}</h2>
                                <p>{{ __('main.pdf_print_identity_help') }}</p>
                            </div>
                        </header>
                        ...
                        <div class="pdf-settings-layout">
                            ...
                            <div class="pdf-theme-preview" style="--preview-primary: {{ old('pdf_primary_color', $settings->pdf_primary_color) }}; --preview-accent: {{ old('pdf_accent_color', $settings->pdf_accent_color) }}; --preview-tint: {{ old('pdf_tint_color', $settings->pdf_tint_color) }};">
                                <strong>{{ __('main.pdf_document_preview') }}</strong>
                                <span class="pdf-preview-title">{{ app_brand_short_name() }}</span>
                    ...
                    <section class="settings-panel">
                        <header class="settings-panel-heading">
                            ...
                            <div>
                                <h2>{{ __('main.menu_access_management') }}</h2>
                                <p>{{ __('main.menu_access_management_help') }}</p>
                            </div>
                        </header>

                        <div class="settings-info">
                            <i class="bi bi-info-circle" aria-hidden="true"></i>
                            {{ __('main.all_access_for_admins_notice') }}
                        </div>
