        ...
        <main class="dashboard-main">
            ...
            <section class="dashboard-content module-dashboard-page accounting-notifications-page archiving-page">
                ...
                <div class="report-section-tabs notification-tabs">
                    <a class="{{ $status !== 'unread' ? 'active' : '' }}" href="{{ route('main.archiving.notifications', [$company, $site]) }}">{{ __('main.all_notifications') }}</a>
                    <a class="{{ $status === 'unread' ? 'active' : '' }}" href="{{ route('main.archiving.notifications', [$company, $site, 'status' => 'unread']) }}">{{ __('main.unread_notifications') }}</a>
                </div>

                <section class="notifications-table-card">
                    @forelse ($notifications as $notification)
                        @php($isRead = $notification->isReadBy($user))
                        <a class="notification-row {{ $isRead ? '' : 'unread' }}" href="{{ route('main.archiving.notifications.show', [$company, $site, $notification]) }}">
