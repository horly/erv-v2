...
<body class="accounting-module-body document-management-module-body">
    <div class="dashboard-shell main-shell accounting-shell document-management-shell" data-theme="light">
        @include('main.modules.document-management.partials.sidebar', ['activeDocumentManagementPage' => 'notifications'])
        ...
        <main class="dashboard-main">
            ...
            <section class="dashboard-content module-dashboard-page accounting-notifications-page document-management-page">
                ...
                <div class="report-section-tabs notification-tabs">
                    <a class="{{ $status !== 'unread' ? 'active' : '' }}" href="{{ route('main.document-management.notifications', [$company, $site]) }}">{{ __('main.all_notifications') }}</a>
                    <a class="{{ $status === 'unread' ? 'active' : '' }}" href="{{ route('main.document-management.notifications', [$company, $site, 'status' => 'unread']) }}">{{ __('main.unread_notifications') }}</a>
                </div>

                <section class="notifications-table-card">
                    @forelse ($notifications as $notification)
                        @php($isRead = $notification->isReadBy($user))
                        <a class="notification-row {{ $isRead ? '' : 'unread' }}" href="{{ route('main.document-management.notifications.show', [$company, $site, $notification]) }}">
