    ...
    <div class="dashboard-shell main-shell accounting-shell human-resources-shell" data-theme="light">
        ...
        <main class="dashboard-main">
            ...
            <section class="dashboard-content module-dashboard-page accounting-notifications-page human-resources-page">
                <a class="back-link" href="{{ route('main.human-resources.notifications', [$company, $site]) }}">
                ...
                <section class="settings-panel notification-detail-grid">
                    <div>
                        <span>{{ __('main.actor') }}</span>
                        <strong>{{ $notification->actor?->name ?: __('main.system_user') }}</strong>
                    </div>
                    <div>
                        <span>{{ __('main.reference') }}</span>
                        <strong>{{ $notification->subject_reference ?: '-' }}</strong>
                    </div>
                    <div>
                        <span>{{ __('main.module') }}</span>
                        <strong>{{ $moduleLabel ?? $notification->module_key }}</strong>
                    </div>
                    <div>
                        <span>{{ __('main.date') }}</span>
                        <strong>{{ $notification->occurred_at?->translatedFormat('d/m/Y H:i') }}</strong>
                    </div>
                </section>

                @if ($moduleUrl)
                    <div class="settings-actions">
                        <a class="primary-action" href="{{ $moduleUrl }}">
                            <i class="bi bi-box-arrow-up-right" aria-hidden="true"></i>
                            {{ __('main.open_related_module') }}
                        </a>
                    </div>
                @endif
            </section>
