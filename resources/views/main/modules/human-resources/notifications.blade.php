...
<body class="accounting-module-body human-resources-module-body">
    <div class="dashboard-shell main-shell accounting-shell human-resources-shell" data-theme="light">
        @include('main.modules.human-resources.partials.sidebar', ['activeHumanResourcesPage' => 'notifications'])
        ...
        <main class="dashboard-main">
            ...
            <section class="dashboard-content module-dashboard-page accounting-notifications-page human-resources-page">
                ...
                <div class="report-section-tabs notification-tabs">
                    <a class="{{ $status !== 'unread' ? 'active' : '' }}" href="{{ route('main.human-resources.notifications', [$company, $site]) }}">{{ __('main.all_notifications') }}</a>
                    <a class="{{ $status === 'unread' ? 'active' : '' }}" href="{{ route('main.human-resources.notifications', [$company, $site, 'status' => 'unread']) }}">{{ __('main.unread_notifications') }}</a>
                </div>

                <section class="notifications-table-card">
                    @forelse ($notifications as $notification)
                        @php($isRead = $notification->isReadBy($user))
                        <a class="notification-row {{ $isRead ? '' : 'unread' }}" href="{{ route('main.human-resources.notifications.show', [$company, $site, $notification]) }}">
