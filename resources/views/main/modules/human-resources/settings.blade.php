...
<head>
    ...
    <title>{{ __('main.hr_settings') }} | {{ app_brand_name() }}</title>
...
<body class="accounting-module-body human-resources-module-body">
    ...
    <div class="dashboard-shell main-shell accounting-shell human-resources-shell" data-theme="light">
        @include('main.modules.human-resources.partials.sidebar', ['activeHumanResourcesPage' => 'settings'])

        <main class="dashboard-main">
            <header class="dashboard-topbar">
                <div>
                    <h1>{{ __('main.hr_settings') }}</h1>
            ...
            <section class="dashboard-content module-dashboard-page accounting-settings-page human-resources-page">
                ...
                <section class="page-heading">
                    <div>
                        <h1>{{ __('main.hr_settings') }}</h1>
                        <p>{{ __('main.hr_module_settings_subtitle') }}</p>
                    </div>
                </section>
                ...
                <form method="POST" action="{{ route('main.human-resources.settings.update', [$company, $site]) }}" class="accounting-settings-form">
                    @csrf
                    @method('PUT')
                    ...
                    <section class="settings-panel">
                        <header class="settings-panel-heading">
                            ...
                            <div>
                                <h2>{{ __('main.hr_pdf_identity') }}</h2>
                                <p>{{ __('main.hr_pdf_identity_help') }}</p>
                            </div>
                        </header>
                        ...
                        <div class="pdf-settings-layout">
                            ...
                            <div class="pdf-theme-preview" style="--preview-primary: {{ old('pdf_primary_color', $settings->pdf_primary_color) }}; --preview-accent: {{ old('pdf_accent_color', $settings->pdf_accent_color) }}; --preview-tint: {{ old('pdf_tint_color', $settings->pdf_tint_color) }};">
                                <strong>{{ __('main.hr_pdf_preview') }}</strong>
                                <span class="pdf-preview-title">{{ __('main.module_human_resources') }}</span>
                    ...
                    <section class="settings-panel">
                        <header class="settings-panel-heading">
                            ...
                            <div>
                                <h2>{{ __('main.hr_menu_access_management') }}</h2>
                                <p>{{ __('main.hr_menu_access_management_help') }}</p>
                            </div>
                        </header>

                        <div class="settings-info">
                            <i class="bi bi-info-circle" aria-hidden="true"></i>
                            {{ __('main.hr_all_access_for_admins_notice') }}
                        </div>
